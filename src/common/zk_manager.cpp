...
bool ZkManager::init(const std::string& zkAddress)
{
    ...
    if(!_zh)
    {
        std::cerr<<"ZkManager::init failed, because zookeeper_init return a null pointer!"<<std::endl;
        return false;
    }
...
{
    for(std::vector<ZooKeeperListenerPtr>::const_iterator it = _listeners.begin(); it != _listeners.end(); ++it)
    {
        (*it)->handle();
    }
}
