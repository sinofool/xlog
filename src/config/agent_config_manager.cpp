...
AgentConfigManager::AgentConfigManager(const std::string& prx, const ZkManagerPtr& zm, const ClientAdapterPtr& clientAdapter) :
...
bool AgentConfigManager::handle()
{
    ...
    std::vector<std::string>::const_iterator findIt = find(newConfig.begin(), newConfig.end(), _prx);
