...
ClientAdapter::ClientAdapter(const ClientConfigManagerPtr& clientCM) : _clientCM(clientCM)
{
    _ic = Ice::initialize();
}
...
void ClientAdapter::notify(const std::vector<std::string>& agentConfig)
{
    ...
    for(std::vector<std::string>::const_iterator it = prxVec.begin(); it != prxVec.end(); ++it)
    {
        ...
        {
            std::cerr << "ClientAdapter::notify Ice::Exception of prx " << prx->ice_toString() <<std::endl;
        }catch(...)
        {
            std::cerr << "ClientAdapter::notify UnknownException of prx " << prx->ice_toString() <<std::endl;
        }
